...
func FetchInCacheContent(ctx *context.Context, cfg *config.IpfsConfig) (time.Duration, error) {
	cid := ctx.AvailableCids[0]
	...
	// the cache.
	resp, err := client.Get(fmt.Sprintf("https://%s/ipfs/%s", ctx.GatewayHost, cid))
	if err != nil {
		return 0, err
	}
